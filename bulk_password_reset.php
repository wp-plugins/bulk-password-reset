...
if (!class_exists('bulk_password_reset')) {
    class bulk_password_reset {
        ...
        function __construct(){
            ...
            //Filter + set upload folder       
            add_filter('upload_dir', array(&$this, 'set_correct_logfolder') );
            $upload = wp_upload_dir();
            remove_filter('upload_dir', array(&$this, 'set_correct_logfolder') );

        }

        function set_correct_logfolder($upload) {
            $upload['subdir']    = '/bulk-password-reset' . $upload['subdir'];
        ...
        /**
         * @desc Adds the page to the users sub menu
         */
                                    ...
                                    jQuery('#advancedsettings').change(function(){
                                        if (jQuery('#advancedsettings').is(':checked')) {
                                            jQuery('#advancedbprsettings').css("display","block");
                                                if (jQuery('#uselogfile').is(':checked')) {
                                                    jQuery('#logfilesbpr').css("display","block");
                                                }
                                        } else {
                                            jQuery('#advancedbprsettings').css("display","none");
                                            jQuery('#logfilesbpr').css("display","none");
                                        }
                                    }).change();
                                    jQuery('#uselogfile').change(function(){
                                        if (jQuery('#uselogfile').is(':checked')) {
                                            jQuery('#logfilesbpr').css("display","block");
                                        } else {
                                            jQuery('#logfilesbpr').css("display","none");
                    ...
                    if ( isset($_POST['submit']) ) {
                        ...
                        if (isset($_POST['dobulkreset'])) {
                            ...
                            if ( !isset($_POST['send_roles']) || !is_array($_POST['send_roles']) || empty($_POST['send_roles']) ) {
                                    $err_msg =  __('No reset has been executed as no group was selected. You need to select at least one group to let the plugin do his work.', 'bulk_password_reset');
                            }
                            else {
                                ...
                                if (empty($recipients)) {
                                    $err_msg = __('No recipients where found in the selected groups', 'bulk_password_reset');
                                }
                                else {
                            ...
                            if ( $err_msg !='' ) {
                                ...
                                echo "<div id=\"successmessage\" class=\"updated fade\"><p>" . __('Successfully reset the password for the selected role(s).', 'bulk_password_reset') . "</p></div>\n";
                                ...
                                <form action="" method="post" id="bpr-config">
                                    ...
                                    <div id="bprsettings" class="postbox">
                                        ...
                                        <div class="bprbox">
                                            <table class="form-table">
                                                ...
                                                <tr>
                                                    <th valign="top" scrope="row"><label for="dobulkreset"><?php _e('Do the bulk reset:', 'bulk_password_reset') ?></label><br/><small><?php _e('Activate this check box when you actually would like to do the reset, else only the settings are saved.', 'bulk_password_reset') ?></small></th>
                                                    <td valign="top"><input type="checkbox" id="dobulkreset" name="dobulkreset" /></td>
                                                </tr>
                                                <tr>
                                                    <th valign="top" scrope="row"><label for="advancedsettings"><?php _e('Show advanced settings:', 'bulk_password_reset') ?></label><br/><small><?php _e('Only advised for users who want to change the default reset options', 'bulk_password_reset') ?></small></th>
                                    ...
                                    <div id="advancedbprsettings" class="postbox">
                                        ...
                                        <div class="bprbox">
                                            <table class="form-table">
                                                <tr>
                                                    <th valign="top" scrope="row"><label for="hardreset"><?php _e('Direct password reset', 'bulk_password_reset') ?>:</label><br/><small><?php _e('Not recommended, as this immediately changes the password of the users without confirmation e-mail.','bulk_password_reset') ?></small></th>
                                                ...
                                                <tr>
                                                    <th valign="top" scrope="row"><label for="uselogfile"><?php _e('Use logfile?', 'bulk_password_reset') ?>:</label><br/><small><?php _e('Write the result of the password reset to a text log file for track and trace','bulk_password_reset') ?></small></th>
                                                    <td valign="top"><input type="checkbox" id="uselogfile" name="uselogfile" <?php echo checked($options['uselogfile'],true,false); ?> /></td>
                                                </tr>
                                                <tr>
                                    ...
                                    <!-- Show logfiles for download -->
                                    <div id="logfilesbpr" class="postbox">
                                        <div class="handlediv" title="Click to toggle"><br /></div>
                                        <h3 class="hndle"><span><?php _e('Bulk Password Reset - Log files', 'bulk_password_reset') ?></span></h3>
                                        <div class="bprbox">
                                            <p><?php _e('Down below you will find an overview of the log files belonging to the earlier resets', 'bulk_password_reset') ?></p>
                                            <?php
                                            //path to directory to scan
                                            $directory = $upload['path'];

                                            //get all files with a .log extension.
                ...
                function news() {
                    ...
                    //Get the SimplePie feed from feedburner!
                    $rss = fetch_feed( 'http://feeds.feedburner.com/rubenwoudsma' );
                    ...
                    $content = '<ul>';
                    if ( $maxitems == 0 ) {
                        $content .= '<li class="brp-authorinfo">' . __('Nothing to say...','bulk_password_reset') . '</li>';
                    } else {
                        foreach ( $rss_items as $item ) {
                            $content .= '<li class="brp-authorinfo">';
                            $content .= '<a class="rsswidget" href="'.esc_url( $item->get_permalink() ).'">'. esc_html( $item->get_title() ) .'</a> ';
                            $content .= '</li>';
                        }
                ...
                /**
                 * @desc Write message to debug.log based upon wp-config.php WP_DEBUG setting
                 */
                function log_msg ( $message ) {
                   
                    //Only write to debug.log is WP_DEBUG is set
                    if( WP_DEBUG === true ){
                        //Check to see if array needs to be written, then use print_r
                ...
                function sent_activation_mail($user_login) {
                    ...
                    if ( $message && !wp_mail($user_email, $title, $message) ) {
                        die('<p>' . __('The email could not be sent.', 'bulk_password_reset') . "<br />\n" . __('Possible reason: your host may have disabled the mail() function...', 'bulk_password_reset') . '</p>');
                        $this->log_msg ('ERROR: Not able to sent activation message to ' . $user_login);
                    } else {
                        $this->log_msg ('INFO: Successfully sent activation message to ' . $user_login);
                    }
                ...
                function reset_password($user_login) {
                    ...
                    if ( $message && !wp_mail($user->user_email, $title, $message) ) {
                        ...
                        $this->log_msg ('ERROR: Not able to sent reset password message to ' . $user_login);
                    } else {
                        $this->log_msg ('INFO: Successfully sent reset password message to ' . $user_login);
                    }
